#Gradients untuk operator di array_ops.py.
...
def _ConcatGradHelper(op, grad, start_value_index, end_value_index, dim_index):
  ...
  if isinstance(grad, ops.Tensor):
    ...
    if concat_dim_static is None:
      raise ValueError("Hanya dapat menghitung gradient")
    if concat_dim_static < 0:
      rank = tensor_util.constant_value(array_ops.rank(input_values[0]))
      if rank is None:
        raise ValueError("Hanya dapat menghitung gradien dengan concat_dim_static negatif")
