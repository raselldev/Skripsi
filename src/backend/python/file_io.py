...
class FileIO(object):
  ...
  def _prewrite_check(self):
    if not self._writable_file:
      ...
      with errors.raise_exception_on_not_ok_status() as status:
        self._writable_file = pywrap_backend.CreateWritableFile(
  ...
  def seek(self, offset=None, whence=0, position=None):
    ...
    with errors.raise_exception_on_not_ok_status() as status:
      ...
      pywrap_backend.Set_TF_Status_from_Status(status, ret_status)
...
def delete_file(filename):
  ...
  """
  with errors.raise_exception_on_not_ok_status() as status:
    pywrap_backend.DeleteFile(compat.as_bytes(filename), status)
...
def create_dir(dirname):
  ...
  """
  with errors.raise_exception_on_not_ok_status() as status:
    pywrap_backend.CreateDir(compat.as_bytes(dirname), status)
...
def recursive_create_dir(dirname):
  ...
  """
  with errors.raise_exception_on_not_ok_status() as status:
    pywrap_backend.RecursivelyCreateDir(compat.as_bytes(dirname), status)
...
def delete_recursively(dirname):
  ...
  """
  with errors.raise_exception_on_not_ok_status() as status:
    pywrap_backend.DeleteRecursively(compat.as_bytes(dirname), status)
...
def is_directory(dirname):
  ...
  return pywrap_backend.IsDirectory(compat.as_bytes(dirname), status)
...
def list_directory(dirname):
  ...
  with errors.raise_exception_on_not_ok_status() as status:
    ...
    return [
        ...
        for filename in pywrap_backend.GetChildren(
