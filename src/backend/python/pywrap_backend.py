...
class EventsWriter(_object):
    ...
    def WriteEvent(self, event):
      from backend.core.util.event_pb2 import Event
...
def do_quantize_training_on_graphdef(input_graph, num_bits):
  ...
  """
  #from backend.core.framework.graph_pb2 import GraphDef
  from backend.python.framework import errors
  with errors.raise_exception_on_not_ok_status() as status:
    graph = GraphDef()
...
def IsSequenceForData(o):
    ...
    NOTE(mrry): This differs from `backend.python.util.nest.is_sequence()`,
