...
def _TanhGrad(op, grad):
  ...
  with ops.control_dependencies([grad]):
    y = math_ops.conj(y)
    return math_ops.tanh_grad(y, grad)
...
def _SigmoidGrad(op, grad):
  ...
  with ops.control_dependencies([grad]):
    y = math_ops.conj(y)
    return math_ops.sigmoid_grad(y, grad)
...
def _MatMulGrad(op, grad):
  ...
  if not t_a and not t_b:
    grad_a = math_ops.mat_mul(grad, b, transpose_b=True)
    grad_b = math_ops.mat_mul(a, grad, transpose_a=True)
  elif not t_a and t_b:
    grad_a = math_ops.mat_mul(grad, b)
    grad_b = math_ops.mat_mul(grad, a, transpose_a=True)
  elif t_a and not t_b:
    grad_a = math_ops.mat_mul(b, grad, transpose_b=True)
    grad_b = math_ops.mat_mul(a, grad)
  elif t_a and t_b:
    grad_a = math_ops.mat_mul(b, grad, transpose_a=True, transpose_b=True)
    grad_b = math_ops.mat_mul(grad, a, transpose_a=True, transpose_b=True)
  return grad_a, grad_b
